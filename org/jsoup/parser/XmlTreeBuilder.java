...
public class XmlTreeBuilder extends TreeBuilder
{
    ...
    if (bogus)
    {
      ...
      if ((data.length() > 1) && ((data.startsWith("!")) || (data.startsWith("?")))) {
        ...
        if (doc.childNodeSize() > 0) {
          ...
          insert = new org.jsoup.nodes.XmlDeclaration(settings.normalizeTag(el.tagName()), data.startsWith("!"));
  ...
  void insert(Token.Character token) {
    ...
    insertNode(token.isCData() ? new org.jsoup.nodes.CDataNode(data) : new org.jsoup.nodes.TextNode(data));
  }
